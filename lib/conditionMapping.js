...
var CONDITION_HANDLE = {
    ...
    mime: (condition, type) => {
        if (type == "ClientFile")
            return ` && (input.type == ${JSON.stringify(condition)})`;
        else return "";
    },
    lt: (condition) => {
    ...
    eq: (condition) => {
        return ` && (input == ${JSON.stringify(condition)})`;
    },
    ne: (condition) => {
        return ` && (input != ${JSON.stringify(condition)})`;
    },
...
function getChecker(argsName="input", conditionMap) {
    ...
    var checkerRaw = `function checker(input){
        ...
        if(!isMatch){
            throw new HTTPMessage(
                ...
                \`Invalid param '${argsName}', check if argument satisfying conditions ${prettyCondition}\`
            );
        };
        return isMatch;
    }; checker;`
